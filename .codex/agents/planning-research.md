...
description: Searches completed plans and past features for reusable patterns and prior decisions.
             Use at the start of /planning. Trigger phrases: "search past plans", "find prior
             decisions", "look for reusable patterns", "research past features", "what have
             we done before", "find completed plan references".
model: haiku
---
...
2. **Scan completed plans**:
   ...
   - Also scan `.agents/features/*/task-{N}.done.md` for task-level patterns
   - Read each completed plan's Feature Description, Solution Statement, and Patterns to Follow sections
   - From completed task briefs, read Step-by-Step Tasks and Handoff Notes for implementation patterns
   - Identify plans/briefs sharing similar technologies, patterns, or architectural concerns
...
- Only reference completed artifacts (`.done.md`) — active plans are in-progress and unreliable
