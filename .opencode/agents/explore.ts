...
export const EXPLORE_METADATA: AgentMetadata = AGENT_REGISTRY["explore"]
 ...
 */
function buildExploreApproachSteps(): string {
  const steps = [
 ...
 */
export function createExplorePrompt(context: AgentPromptContext): BuiltPrompt {
 ...
 */
function buildToolPriority(): string {
 ...
 */
function buildSearchPatterns(): string {
 ...
 */
export function createExploreAgent(model: string) {
  const metadata = EXPLORE_METADATA
  return {
    name: metadata.name,
    instructions: buildExploreSystemPrompt(
      metadata,
      buildRoleContext(),
      buildExploreApproachSteps(),
      buildToolPriority(),
      buildOutputFormat(),
      buildSearchPatterns()
    ),
