...
export const HEPHAESTUS_METADATA: AgentMetadata = AGENT_REGISTRY["hephaestus"]
 ...
 */
function buildHephaestusApproachSteps(): string {
  const steps = [
 ...
 */
export function createHephaestusPrompt(context: AgentPromptContext): BuiltPrompt {
 ...
 */
function buildHephaestusSystemPrompt(
 ...
 */
function buildAutonomyLevel(): string {
 ...
 */
export function createHephaestusAgent(model: string) {
  const metadata = HEPHAESTUS_METADATA
  return {
    name: metadata.name,
    instructions: buildHephaestusSystemPrompt(
      metadata,
      buildRoleContext(),
      buildHephaestusApproachSteps(),
      buildAutonomyLevel(),
      buildOutputFormat()
    ),
    model,
