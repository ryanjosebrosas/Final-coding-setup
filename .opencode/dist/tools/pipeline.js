...
// ============================================================================
import { readHandoff, writeHandoff, updateHandoff, } from "../pipeline/handoff";
import { getArtifactDiscoveryResult, } from "../pipeline/artifacts";
import { validateTransition, getValidNextStates, getStatusDescription, } from "../pipeline/state-machine";
import { listCommands, suggestNextCommand, } from "../pipeline/commands";
/**
 ...
 */
function handleStatus(workspaceDir) {
 ...
 */
function handleNext(workspaceDir) {
    const handoff = readHandoff(workspaceDir);
    if (!handoff) {
        return `# No Active Pipeline
 ...
 */
function handleAdvance(workspaceDir, targetStatus) {
    if (!targetStatus) {
...
No handoff file found. Start with \`/planning {feature}\`.`;
    ...
    const result = validateTransition(handoff.status, targetStatus);
    if (!result.success) {
...
Run \`pipeline next\` to see valid transitions.`;
    ...
    const updated = updateHandoff(handoff, { status: targetStatus });
...
Failed to write handoff file. Check file permissions.`;
    ...
    const toDesc = getStatusDescription(targetStatus);
    const suggestion = suggestNextCommand(targetStatus, handoff.feature);
    const lines = [
        `# State Advanced`,
        ``,
 ...
 */
function handleArtifacts(workspaceDir, feature) {
    if (!feature) {
 ...
 */
function handleCommands(workspaceDir) {
