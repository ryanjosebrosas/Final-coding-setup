 ...
 */
export function writeHandoff(workspaceDir, handoff) {
...
function parseHandoffMarkdown(content) {
    ...
    for (const line of lines) {
        ...
        if (trimmed.startsWith("- **Last Command**:")) {
            ...
            handoff.status = extractValue(trimmed);
        }
 ...
 */
function serializeHandoffMarkdown(handoff) {
    const lines = [
        "# Pipeline Handoff",
        "<!-- Auto-updated by pipeline commands. Read by /prime. Do not edit manually. -->",
        "",
 ...
 */
function parseTaskIndexRow(line) {
 ...
 */
export function createHandoff(feature, lastCommand, nextCommand, status = "awaiting-execution") {
    return {
        lastCommand,
        feature,
        nextCommand,
 ...
 */
export function updateHandoff(handoff, updates) {
    return {
        ...handoff,
        ...updates,
        timestamp: new Date().toISOString(),
 ...
 */
export function hasPendingWork(handoff) {
    if (!handoff)
        return false;
    // Terminal states have no pending work
    if (handoff.status === "pr-open")
        return false;
