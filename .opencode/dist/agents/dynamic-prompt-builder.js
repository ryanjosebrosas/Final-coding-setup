 ...
 */
export function buildAvailableAgentsSummary() {
 ...
 */
function getCategoryPromptAppends() {
    const config = loadCategories();
    const appends = {};
    for (const [name, def] of Object.entries(config.categories)) {
 ...
 */
export function buildCategorySkillsDelegationGuide(category, skills = []) {
 ...
 */
function buildSystemPrompt(agentName, agent) {
    if (!agent) {
 ...
 */
function buildSkillsInjection(skillNames) {
 ...
 */
export function getAgentPromptTemplate(agentName) {
    const templates = {
...
//# sourceMappingURL=dynamic-prompt-builder.js.map