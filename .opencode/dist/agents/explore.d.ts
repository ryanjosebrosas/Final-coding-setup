import { type AgentMetadata } from "./registry";
import { type AgentPromptContext, type BuiltPrompt } from "./prompt-builder";
export declare const EXPLORE_METADATA: AgentMetadata;
/**
 * Build the complete prompt for Explore agent.
 */
export declare function createExplorePrompt(context: AgentPromptContext): BuiltPrompt;
/**
 * Factory function compatible with OhMyOpenCode pattern.
 */
export declare function createExploreAgent(model: string): {
    name: string;
    instructions: string;
    model: string;
    temperature: number;
    mode: import("./types").AgentMode;
    permissions: import("./registry").AgentPermissions;
    fallbackChain: string[];
};
