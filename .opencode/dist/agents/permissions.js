 ...
 */
export function getPermissions(agentName) {
 ...
 */
export function canUseTool(agentName, toolName) {
 ...
 */
export function getDeniedToolsList(agentName) {
    const permissions = getPermissions(agentName);
    const denied = [];
    if (!permissions.writeFile)
        denied.push("Write");
    if (!permissions.editFile)
        denied.push("Edit");
    if (!permissions.bash)
        denied.push("bash");
    if (!permissions.task)
        denied.push("task");
    if (!permissions.archon_mcp)
        denied.push("archon_*");
    if (!permissions.webfetch)
        denied.push("webfetch", "web-reader_*");
    if (!permissions.websearch)
        denied.push("web-search-*");
    return denied;
}
/**
 * Check if agent is read-only.
 */
export function isReadOnly(agentName) {
    const permissions = getPermissions(agentName);
 ...
 */
export function canDelegate(agentName) {
    const permissions = getPermissions(agentName);
 ...
 */
export function enforcePermission(agentName, toolName) {
    if (!canUseTool(agentName, toolName)) {
        const level = getPermissionLevel(agentName);
        throw new Error(`Agent ${agentName} (${level}) is not permitted to use tool: ${toolName}. ` +
 ...
 */
export function filterAvailableTools(agentName, allTools) {
    const allowed = [];
    const denied = [];
