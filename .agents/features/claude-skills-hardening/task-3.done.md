...
Create `.claude/skills/prime/SKILL.md` — a knowledge framework for context loading quality,
stack detection completeness, and pending work detection accuracy.

## Scope

- **File to create**: `.claude/skills/prime/SKILL.md`
- **Out of scope**: Do NOT modify `.claude/commands/prime.md`
- **Dependencies**: None

## Prior Task Context

Tasks 1-2 created mvp and prd skills. Pattern established: YAML frontmatter + When/
Anti-Patterns/Key Rules/Related Commands. Skills add DEPTH (quality criteria, anti-patterns),
not workflow duplication.

## Context References

### Reference: .claude/commands/prime.md (key sections)
...
git status --short. If dirty: warn with file list.

## Step 1: Detect Context Mode
Check for code dirs. If found → Codebase Mode. If not → System Mode.

## Step 2: Auto-Detect Tech Stack
Package manifests → language + framework
Linter config → L1 validation command
Type checker config → L2 validation command
Test runner config → L3/L4 validation commands
Entry point detection → project overview

## Step 3: Write/Update .claude/config.md
Create if absent. Read if exists (user overrides win).

## Step 3.5: Detect Pending Work
Source 1: Handoff file (.agents/context/next-command.md)
Source 2: Artifact scan (.agents/features/*/...)
Merge logic:
  - Handoff + artifact scan agree → use handoff
  - Handoff contradicts artifacts → artifact scan wins (note "Handoff stale")
  - No handoff → use artifact scan only

## Step 4: Assemble Report
System Mode: Branch, Status, Recent Work, Memory Context, Pending Work
Codebase Mode: + Tech Stack, Project Overview, Archon Status
```

## Step-by-Step Tasks

### IMPLEMENT: Create `.claude/skills/prime/SKILL.md`
...
```bash
grep -c "name: prime" .claude/skills/prime/SKILL.md
grep -c "When This Skill Applies" .claude/skills/prime/SKILL.md
grep -c "Key Rules" .claude/skills/prime/SKILL.md
grep -c "Anti-Patterns" .claude/skills/prime/SKILL.md
```
...
# L1
grep -c "name: prime" .claude/skills/prime/SKILL.md
grep -c "compatibility: claude-code" .claude/skills/prime/SKILL.md
grep -c "When This Skill Applies" .claude/skills/prime/SKILL.md
grep -c "Key Rules" .claude/skills/prime/SKILL.md
# L2-L5: N/A
```
...
### Implementation
- [ ] `.claude/skills/prime/SKILL.md` exists
- [ ] Frontmatter has all 4 fields
- [ ] Covers dirty state awareness, stack detection completeness, pending work accuracy
- [ ] Has Anti-Patterns and Key Rules sections
- [ ] Does NOT duplicate command steps

## Handoff Notes

Task 4 creates `.claude/skills/council/SKILL.md`. Focus: perspective selection quality,
genuine argument discipline (not strawman), synthesis vs. picking a winner.

## Completion Checklist

- [ ] `.claude/skills/prime/SKILL.md` created
