...
Create `.claude/skills/code-review-fix/SKILL.md` — a knowledge framework for disciplined
fix application: minimal change principle, severity ordering, per-fix verification, and
scope discipline.

## Scope

- **File to create**: `.claude/skills/code-review-fix/SKILL.md`
- **Out of scope**: Do NOT modify `.claude/commands/code-review-fix.md`
- **Dependencies**: None

## Prior Task Context

Tasks 1-6 complete. Task 7 is the companion skill to code-review — where code-review
finds issues, code-review-fix applies them with discipline.

## Context References

### Reference: .claude/commands/code-review-fix.md (key sections)

```markdown
## Hard Entry Gate
Requires review file. Never fix without a review artifact.
...
Lint → Types → Tests after ALL fixes applied.
...
- Respect scope: Don't fix Minor if scope is critical+major
- Report unresolvable: If a fix introduces new problems, stop and report
```

## Step-by-Step Tasks

### IMPLEMENT: Create `.claude/skills/code-review-fix/SKILL.md`
...
```bash
grep -c "name: code-review-fix" .claude/skills/code-review-fix/SKILL.md
grep -c "When This Skill Applies" .claude/skills/code-review-fix/SKILL.md
grep -c "Key Rules" .claude/skills/code-review-fix/SKILL.md
grep -c "Anti-Patterns" .claude/skills/code-review-fix/SKILL.md
```
...
# L1
grep -c "name: code-review-fix" .claude/skills/code-review-fix/SKILL.md
grep -c "compatibility: claude-code" .claude/skills/code-review-fix/SKILL.md
grep -c "When This Skill Applies" .claude/skills/code-review-fix/SKILL.md
grep -c "Key Rules" .claude/skills/code-review-fix/SKILL.md
# L2-L5: N/A
```
...
### Implementation
- [ ] `.claude/skills/code-review-fix/SKILL.md` exists
- [ ] Covers minimal change principle with examples
- [ ] Covers severity ordering rationale
- [ ] Covers per-fix verification (not batch)
- [ ] Covers unresolvable case handling
- [ ] Has Anti-Patterns and Key Rules sections

## Handoff Notes

Task 8 creates `.claude/skills/code-loop/SKILL.md`. Focus: checkpoint discipline,
escape hatch criteria (when to stop and escalate to /planning), loop exit conditions,
completion sweep.

## Completion Checklist

- [ ] `.claude/skills/code-review-fix/SKILL.md` created
