...
Create `.claude/skills/pr/SKILL.md` — a knowledge framework for PR creation quality:
branch isolation rationale, cherry-pick scoping, PR body quality standards, and
report-driven scope detection.

## Scope

- **File to create**: `.claude/skills/pr/SKILL.md`
- **Out of scope**: Do NOT modify `.claude/commands/pr.md`
- **Dependencies**: None

## Prior Task Context

Tasks 1-10 complete. This is the final task (11/11). PR is the last step in the pipeline —
it publishes the feature for review and merge.

## Context References

### Reference: .claude/commands/pr.md (key sections)
...
Step 2a: Identify feature (from $ARGUMENTS, latest report.done.md, or commit message)
Step 2b: Locate report and parse files touched (Meta Information section)
Step 2c: Intersect with unpushed commits (git log REMOTE/MAIN..HEAD -- {FILES})

## Step 4: Create Isolated Feature Branch
CRITICAL: Branch from remote main (not local HEAD)
1. git checkout -b {branch-name} {REMOTE}/{MAIN_BRANCH}
2. git cherry-pick {commits} — oldest first
3. git push {REMOTE} {branch-name} -u
4. git checkout {original-branch}

## Step 5: Generate PR Title and Body
Title: type(scope): description (max 72 chars)
Body sections: What, Why, Changes, Testing, Notes
Context from: execution report + code review done files

## Key Rules
- Branch from remote main (not local HEAD)
- Cherry-pick brings only selected commits
...
### IMPLEMENT: Create `.claude/skills/pr/SKILL.md`
...
```bash
grep -c "name: pr" .claude/skills/pr/SKILL.md
grep -c "When This Skill Applies" .claude/skills/pr/SKILL.md
grep -c "Key Rules" .claude/skills/pr/SKILL.md
grep -c "Anti-Patterns" .claude/skills/pr/SKILL.md
```
...
# L1
grep -c "name: pr" .claude/skills/pr/SKILL.md
grep -c "compatibility: claude-code" .claude/skills/pr/SKILL.md
grep -c "When This Skill Applies" .claude/skills/pr/SKILL.md
grep -c "Key Rules" .claude/skills/pr/SKILL.md
# L2-L5: N/A
```
...
- [ ] Covers branch isolation rationale with concrete example
- [ ] Covers cherry-pick scoping (three-step approach)
- [ ] Covers PR body quality by section
- [ ] Has Anti-Patterns and Key Rules sections

## Handoff Notes

This is the final task (11/11). After this, the executor should:
1. Run L1 validation on all 11 skill files
2. Write the execution report to `.agents/features/claude-skills-hardening/report.md`
3. Update the pipeline handoff to awaiting-review

## Completion Checklist

- [ ] `.claude/skills/pr/SKILL.md` created
- [ ] All grep validations pass
- [ ] `task-11.md` → `task-11.done.md` rename completed
- [ ] This is the final task — rename `plan.md` → `plan.done.md` after all briefs done
