...
Create `.claude/skills/code-loop/SKILL.md` — a knowledge framework for automated fix loop
discipline: checkpoint system, escape hatch criteria, loop exit conditions, and completion
sweep standards.

## Scope

- **File to create**: `.claude/skills/code-loop/SKILL.md`
- **Out of scope**: Do NOT modify `.claude/commands/code-loop.md`
- **Dependencies**: None

## Prior Task Context

Tasks 1-7 complete. The review/fix skills (code-review, code-review-fix) are done.
Code-loop is the orchestrator that runs them in an automated cycle.

## Context References

### Reference: .claude/commands/code-loop.md (key sections)
...
## Fix Loop
1. Run /code-review → save to review-{N}.md
2. Check findings: 0 → exit; Minor only → ask user; Critical/Major → fix
3. Fix with /code-review-fix (scope: critical+major default)
4. Run full validation pyramid
5. Check for unfixable errors

## Loop Exit Conditions
0 issues + validation passes → hand off to /commit
Only Minor issues → ask user
Unfixable error → stop, report

## Escape Hatch: /planning → /execute
When: fix requires 5+ files, new abstractions, or API surface changes
Steps: create fix plan via /planning → execute via /execute → mark review .done.md

## Completion Sweep
On clean exit:
- loop-report-{N}.md → loop-report-{N}.done.md
- review-{N}.md → review-{N}.done.md (if not already done)
- fixes-{N}.md → fixes-{N}.done.md (if applied)
```

## Step-by-Step Tasks

### IMPLEMENT: Create `.claude/skills/code-loop/SKILL.md`
...
```bash
grep -c "name: code-loop" .claude/skills/code-loop/SKILL.md
grep -c "When This Skill Applies" .claude/skills/code-loop/SKILL.md
grep -c "Key Rules" .claude/skills/code-loop/SKILL.md
grep -c "Anti-Patterns" .claude/skills/code-loop/SKILL.md
```
...
# L1
grep -c "name: code-loop" .claude/skills/code-loop/SKILL.md
grep -c "compatibility: claude-code" .claude/skills/code-loop/SKILL.md
grep -c "When This Skill Applies" .claude/skills/code-loop/SKILL.md
grep -c "Key Rules" .claude/skills/code-loop/SKILL.md
# L2-L5: N/A
```
...
### Implementation
- [ ] `.claude/skills/code-loop/SKILL.md` exists
- [ ] Covers checkpoint discipline with specific format
- [ ] Covers escape hatch criteria (specific thresholds)
- [ ] Covers loop exit conditions with judgment standards
- [ ] Covers completion sweep rules
- [ ] Has Anti-Patterns and Key Rules sections

## Handoff Notes

Task 9 creates `.claude/skills/system-review/SKILL.md`. Focus: scoring methodology depth,
divergence classification judgment, memory suggestion quality, specificity of system
improvement actions.

## Completion Checklist

- [ ] `.claude/skills/code-loop/SKILL.md` created
