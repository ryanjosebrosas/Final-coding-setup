...
Create `.claude/skills/execute/SKILL.md` — a knowledge framework for plan execution quality:
divergence classification depth, self-review discipline, validation pyramid enforcement,
and one-brief-per-session discipline.

## Scope

- **File to create**: `.claude/skills/execute/SKILL.md`
- **Out of scope**: Do NOT modify `.claude/commands/execute.md`
- **Dependencies**: None

## Prior Task Context

Tasks 1-4 created discovery phase skills (mvp, prd, prime, council). Pattern established and
stable. Task 5 begins the execution phase skills.

## Context References

### Reference: .claude/commands/execute.md (key methodology sections)

```markdown
## Hard Entry Gate
/execute is plan-bound only. Requires .agents/features/ plan file.
Never execute from chat intent alone.

## Divergence Classification
Good Divergence ✅ — Plan limitations:
...
Bad Divergence ❌ — Execution issues:
  - Ignored explicit constraints in plan
  - Created new architecture instead of following patterns
  - Shortcuts introducing technical debt
...
## Self-Review (Plan Cross-Check)
5a. Task-by-task: was ACTION on correct TARGET? does implementation match IMPLEMENT?
    was VALIDATE run and passed?
5b. Acceptance criteria: concrete evidence for each?
5c. File inventory: all planned files created? unplanned files?
5d. Summary format: Tasks/Files/Acceptance/Validation/Gaps/Verdict

## Validation Policy (non-skippable)
Every loop: full validation depth for the current slice.
Minimum: syntax → types → unit tests → integration → manual.
Do not treat single checks as sufficient.

## Key rule: ONE phase/brief per session
```

## Step-by-Step Tasks

### IMPLEMENT: Create `.claude/skills/execute/SKILL.md`
...
```bash
grep -c "name: execute" .claude/skills/execute/SKILL.md
grep -c "When This Skill Applies" .claude/skills/execute/SKILL.md
grep -c "Key Rules" .claude/skills/execute/SKILL.md
grep -c "Anti-Patterns" .claude/skills/execute/SKILL.md
```
...
# L1
grep -c "name: execute" .claude/skills/execute/SKILL.md
grep -c "compatibility: claude-code" .claude/skills/execute/SKILL.md
grep -c "When This Skill Applies" .claude/skills/execute/SKILL.md
grep -c "Key Rules" .claude/skills/execute/SKILL.md
# L2-L5: N/A
```
...
### Implementation
- [ ] `.claude/skills/execute/SKILL.md` exists
- [ ] Covers divergence classification with deep criteria
- [ ] Covers self-review quality standards
- [ ] Covers validation pyramid non-negotiability
- [ ] Covers one-brief-per-session discipline
- [ ] Has Anti-Patterns and Key Rules sections

## Handoff Notes

Task 6 creates `.claude/skills/code-review/SKILL.md`. Focus: severity classification
depth (what makes Critical vs Major vs Minor), review completeness, and what constitutes
evidence in a review finding.

## Completion Checklist

- [ ] `.claude/skills/execute/SKILL.md` created
