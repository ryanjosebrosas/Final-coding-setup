...
Create `.claude/skills/council/SKILL.md` — a knowledge framework for multi-perspective
reasoning quality: perspective selection, genuine argument discipline, and synthesis standards.

## Scope

- **File to create**: `.claude/skills/council/SKILL.md`
- **Out of scope**: Do NOT modify `.claude/commands/council.md`
- **Dependencies**: None

## Prior Task Context

Tasks 1-3 created skills for mvp, prd, prime. Pattern: YAML frontmatter + When/Anti-Patterns/
Key Rules/Related Commands. Skills add DEPTH (quality criteria, anti-patterns, edge case
guidance), not workflow duplication.

## Context References

### Reference: .claude/commands/council.md (full content)
...
## How It Works
Present 3-5 distinct perspectives on the topic. No external dispatch — Claude generates
perspectives internally.

RULE — No Pre-Summarize: Present perspectives FIRST, in full. Do NOT summarize before
the user reads them.

## Action
1. Identify 3-5 distinct perspectives (genuinely different, not slight variations)
   Examples:
...
2. Present each perspective:
   ## Perspective N: {name/role}
   {Full response — 150-300 words. Argue from this viewpoint's values and priorities.}

3. After all perspectives:
   ## Analysis
   - Agreement: {what all perspectives agree on}
   - Conflicts: {where perspectives fundamentally disagree}
   - Key themes: {2-3 recurring themes}

   ## Synthesis
   {Most useful takeaway. Which tradeoffs matter most. What a well-informed decision-maker would conclude.}

## Rules
- Max 1 council per user question
- Perspectives must argue genuinely — not strawman the others
- Synthesis must not pick a winner unless evidence clearly points to one
- If user wants external model input, suggest Codex or another CLI tool
```

## Step-by-Step Tasks

### IMPLEMENT: Create `.claude/skills/council/SKILL.md`
...
```bash
grep -c "name: council" .claude/skills/council/SKILL.md
grep -c "When This Skill Applies" .claude/skills/council/SKILL.md
grep -c "Key Rules" .claude/skills/council/SKILL.md
grep -c "Anti-Patterns" .claude/skills/council/SKILL.md
```
...
# L1
grep -c "name: council" .claude/skills/council/SKILL.md
grep -c "compatibility: claude-code" .claude/skills/council/SKILL.md
grep -c "When This Skill Applies" .claude/skills/council/SKILL.md
grep -c "Key Rules" .claude/skills/council/SKILL.md
# L2-L5: N/A
```
...
### Implementation
- [ ] `.claude/skills/council/SKILL.md` exists
- [ ] Frontmatter has all 4 fields
- [ ] Covers perspective selection quality, genuine argument discipline, synthesis standards
- [ ] Has Anti-Patterns and Key Rules sections
- [ ] Does NOT duplicate command steps

## Handoff Notes

Task 5 creates `.claude/skills/execute/SKILL.md`. Focus: divergence classification depth,
self-review quality, one-brief-per-session discipline, validation pyramid non-negotiability.

## Completion Checklist

- [ ] `.claude/skills/council/SKILL.md` created
